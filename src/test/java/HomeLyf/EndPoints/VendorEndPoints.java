...
public class VendorEndPoints {
	...
	public static Response vendorgetbooking(ITestContext context) {
		String Vtoken = (String) context.getAttribute("Token");

		Response response = given().header("Authorization", "Bearer " + Vtoken).contentType(ContentType.JSON)
	...
	public static Response vendor_startBookingEP(ITestContext context,StartAndComplete_Booking_Payload Payload) {
		...
		Response response = given().header("Authorization", "Bearer " + Vtoken).contentType(ContentType.JSON).body(Payload)
