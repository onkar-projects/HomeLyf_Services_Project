...
public class CustomerEndPoints {
	...
	public static Response customer_MyProfile(ITestContext context) {
		...
		Response response = given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
				.when().get(Routes.customer_MyProfileURL);
	...
	public static Response customer_GetTimeSlot(int addressID,int categoryId,ITestContext context) {
		...
		Response response = given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON).queryParam("addressID", addressID).queryParam("categoryId",categoryId)
				.log().all().when().get(Routes.customer_GetTimeShot);
	...
	public static Response customer_GetBookingEndPoint(ITestContext context, String status,int page, int size) {
		...
		Response response = given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON).queryParam("status",status)
				.queryParam("page", page).queryParam("size", size).when().get(Routes.customer_GetBookingURL);
	...
	public static Response customer_CancelEP(ITestContext context, int bookingId) {
		...
		Response response = given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
				.when().post(Routes.customer_CancelURL, bookingId);
	...
	public static Response customer_SubCategoryEP(ITestContext context, int categoryId ) {
		...
		Response response = given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
				.when().get(Routes.customer_GetSubCategoryURL, categoryId );
	...
	public static Response customer_PaymentModeEP(ITestContext context) {
		...
		Response response = given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
				.when().get(Routes.getPaymentMode);
	...
	public static Response customer_BookingStatusEP(ITestContext context) {
		String token = (String) context.getAttribute("Ctoken");
		Response response = given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
				.when().get(Routes.getBookingStatus);
		return response;
	}
	public static Response customer_GetMyProfileEP(ITestContext context) {
		String token = (String) context.getAttribute("Ctoken");
		Response response = given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
				.when().get(Routes.getMyProfileURL);
		return response;
	}
	public static Response customer_GetCategoryEP(ITestContext context, String postcode, String search) {
		String token = (String) context.getAttribute("Ctoken");
		Response response = given().header("Authorization", "Bearer " + token).queryParam("postcode", postcode).queryParam("search", search).contentType(ContentType.JSON)
				.when().get(Routes.getCategoryURL);
		return response;
	}
	
	public static Response customer_RescheduleEP(ITestContext context, Reschedule_Payload payload) {
		String token = (String) context.getAttribute("Ctoken");
		Response response = given().header("Authorization","Bearer "+ token).contentType(ContentType.JSON).body(payload).log().all()
				  .when().put(Routes.customer_Reschedule);
		return response;
	}
	
	public static Response customer_GetBookingByIdEP(ITestContext context, int bookingId) {
		String token = (String) context.getAttribute("Ctoken");
		Response response = given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON).pathParam("bookingId", bookingId)
				.when().get(Routes.customer_GetBookingIdURL);
		return response;
