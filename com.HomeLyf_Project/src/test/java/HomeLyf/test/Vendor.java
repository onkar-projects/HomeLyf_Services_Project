...
//	int sOTP = 	(int) context.getAttribute("startOTP");
	...
	@Test(enabled = false, description = "Verify that vendor accept booking after enabled that disabled timeslot")
	...
	logger.info("Started creating new Booking.");
		...
		Response response_VendorLogin = UserEndPoints
				.userLogin(CommonMethods.VendorLoginformultipleScenario());
...
//		String status1  = js_vendor_AcceptBookingEP.getString("status");
	...
	@Test(enabled = true, description = "Verify that vendor cannot accept booking after started service ")
		public void vendor_cant_cancel_After_service_Started(ITestContext context) {
			logger.info("Vendor_cant_cancel_After_service_Started");
			logger.info("Started Customer Login test.");
			Response response_CustomerLogin = UserEndPoints
					.userLogin(CommonMethods.CustomerLoginforAcceptBookingAfterEnablingTimeslot());
			response_CustomerLogin.then().log().all();
			...
			response_customer_GetTimeSlot.then().log().all();
			JsonPath js_customer_GetTimeSlot = CommonMethods.jsonToString(response_customer_GetTimeSlot);
			sTime = js_customer_GetTimeSlot.getString("[2].startTime");
			context.setAttribute("StartTime", sTime);
			String eTime = js_customer_GetTimeSlot.getString("[2].endTime");
			System.out.println("Start Time: " + sTime + "\n End Time: " + eTime);
			String statusline_customer_GetTimeSlot = response_customer_GetTimeSlot.getStatusLine();
			Assert.assertEquals(statusline_customer_GetTimeSlot, "HTTP/1.1 200 OK");
			Assert.assertNotNull(response_customer_GetTimeSlot);
		...
		logger.info("Started creating new Booking.");
			...
			// --------------------------------------------------------------------
			logger.info("Vendor accept booking");
			Response response = VendorEndPoints.vendor_AcceptBookingEP(context,id);
		...
		//	Assert.assertEquals(status, "ExpertAssigned");
			...
			Response response1 = CustomerEndPoints.customer_GetBookingByIdEP(context,id);
			response1.then().log().all();
			JsonPath js1 =  CommonMethods.jsonToString(response1);
			int bookingId = js1.get("id");
			context.setAttribute("bookingId", bookingId);
			String status1 = js1.getString("status");
			System.out.println(status1);
			int startOTP = js1.get("startOTP");
			context.setAttribute("startOTP", startOTP);		
			//System.out.println(sOtp);
			int endOTP = js1.get("endOTP");
			context.setAttribute("endOTP", endOTP);
			//System.out.println(eOtp);	
	
			
			Response response2 = VendorEndPoints.vendor_startBookingEP(context, CommonMethods.sendBookingIdandOtp(context));
			...
			// LookUp.customerGetBooking(context);
			//LookUp.vendorgetMybooking(context);
			//int bookingId1 = (int) context.getAttribute("id");
			Response response11 = VendorEndPoints.vendorCancelBooking(context, id);
			JsonPath js11 = CommonMethods.jsonToString(response11);
			//String status11 = js11.get("status");
			Assert.assertEquals(response11.getStatusCode(), 400);
		  logger.info("BookingId " + id + " is not cancelled ");
